...
$(document).ready(function () {
	$('#close_chat').on("click", function (a) {
		if ($('#close_chat i').hasClass('glyphicon-minus')) {
			$("#chat_board").hide(100);
			$('#close_chat i').removeClass('glyphicon-minus').addClass('glyphicon-plus');
		} else {
			$("#chat_board").show(100);
			$('#close_chat i').removeClass('glyphicon-plus').addClass('glyphicon-minus');
		}
	...
	"common": [
		{
			...
			a: [
				{ text: "Indigenous Peoples Rrights", value: "rights", parent: "common" },
				{ text: "Health Insurance", value: "consumer_protection", parent: "common" },
				{ text: "Auto Policy", value: "consumer_protection", parent: "common" },
				{ text: "Life insurance", value: "consumer_protection", parent: "common" }
			]
		}
	],
	"criminal": [
		{
			...
			a: [
				{ text: "Identify theft", value: "consumer_protection", parent: "criminal" },
				{ text: "Credit card fraud", value: "consumer_protection", parent: "criminal" },
				{ text: "Insurance Fraud", value: "consumer_protection", parent: "criminal" },
				{ text: "Securities and Investment Fraud", value: "consumer_protection", parent: "criminal" }
			]
		}
	],
	"civil": [
...
function getQuestionWithAns(level, parent) {
	...
	for(var i=0;i<tags.length;i++){
		resp.a.push( {text : tags[i].text, value: tags[i].tag, parent: tags[i].parent_tag});
	}
}
	else{
		var resp = {
